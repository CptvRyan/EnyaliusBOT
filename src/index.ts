...
async function handleCommand(msg: Discord.Message) {
    ...
    //Split the string into the command of the args
    ...
    for(const commandClass of commands){
        ...
        try{
            ...
            //Check if your command class is the correct one
            if(commandClass.isThisCommand(command)){
                
                //Go to the next iteration of the loop if this isn't the correct command classB
                continue;
            }

            //Pause execution whilest we run the command's code
...
client.login(ConfigFile.config.token).then(() => {
